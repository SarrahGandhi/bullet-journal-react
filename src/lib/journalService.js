...
export async function fetchJournalEntries() {
  ...
  data.forEach((entry) => {
    journalData[entry.date_key] = {
      ...
      goals: entry.goals,
      mood: entry.mood,
      plannerNote: entry.planner_note,
    };
  });
  return journalData;
}
  ...
  const { error } = await supabase.from("journal_entries").upsert(
    {
      ...
      planner_note: entryData.plannerNote || null,
...
export async function addGoal(year, goal) {
  if (!supabase) {
    const existing = JSON.parse(
      localStorage.getItem(`journal-goals-${year}`) || "[]"
    );
    existing.push(goal);
...
export async function saveReflection(year, promptId, content) {
  if (!supabase) {
    const existing = JSON.parse(
      localStorage.getItem(`journal-reflections-${year}`) || "{}"
    );
    existing[promptId] = content;
    localStorage.setItem(
      `journal-reflections-${year}`,
...
export async function saveAllReflections(year, reflections) {
  ...
  const { error } = await supabase.from("reflections").upsert(entries, {
    onConflict: "year,prompt_id",
  });
